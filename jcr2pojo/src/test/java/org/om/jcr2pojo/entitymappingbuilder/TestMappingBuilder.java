...
public class TestMappingBuilder {
   ...
   public void test1() {
      try {
         final Session session = new PropertiesConfiguredJCRSessionFactory().getSession();
         Assert.assertNotNull(session);
         final EntityMappingBuilder entityMappingBuilder = new EntityMappingBuilderImpl(new NodeIdentifierClassNamingStrategy(), new DefaultPropertyNamingStrategy());
         ...
         Assert.assertTrue(entityMapping.getItemMappings().getSize() == 2);
      } catch (final Exception e) {
         e.printStackTrace();
         Assert.fail();
      }
